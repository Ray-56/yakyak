...
impl UserRepository for PgUserRepository {
    ...
    async fn find_by_id(&self, id: i32) -> Result<Option<User>> {
        ...
        let user = sqlx::query_as::<_, User>(
            r#"
            SELECT id, username, password_hash, sip_ha1, realm, display_name, email, enabled, created_at, updated_at
            FROM users
            WHERE id = $1
            "#,
        )
    ...
    async fn change_password(&self, id: i32, data: ChangePassword) -> Result<()> {
        ...
        // Recalculate SIP HA1 with new password
        let new_sip_ha1 = Self::calculate_sip_ha1(&user.username, &user.realm, &data.new_password);

        // Update the password and SIP HA1
        sqlx::query(
            r#"
            UPDATE users
            SET password_hash = $1, sip_ha1 = $2, updated_at = CURRENT_TIMESTAMP
            WHERE id = $3
            "#,
        )
